...
data DBMLNormalizedState = DBMLNormalizedState
...
type DBMLMonad = StateT DBMLNormalizedState (Either Text)

normalize :: Database -> Either Text DBMLNormalizedState
normalize db = execStateT
  (normalizeM db)
...
validateTableInSchema table = do
  ...
  when (tableName table `elem` tableNames) $ lift
    (Left ("Table " `T.append` tableName table `T.append` " already existed"))
...
validateEnumInSchema enum = do
  ...
  when (enumName enum `elem` enumNames) $ lift
    (Left ("Enum " `T.append` enumName enum `T.append` " already existed"))
  ...
  case allDifferent (map enumValue (enumValues enum)) of
    Left val -> lift
      (Left
        (          "Enum value "
        `T.append` val
        `T.append` " already existed in enum "
        `T.append` enumName enum
        )
      )
    _ -> lift (Right ())
...
validateTgInSchema tg = do
  ...
  when (tableGroupName tg `elem` tgNames) $ lift
    (Left
      ("TableGroup " `T.append` tableGroupName tg `T.append` " already existed")
    )
